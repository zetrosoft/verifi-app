...
from web3.contract import Contract as Web3Contract # Alias to avoid conflict with ethers.Contract
import time # For deadline in swap

# --- Mock Data Toggle ---
USE_MOCK_ANALYZE_INPUT = False # Set to True to use mock AI response for /analyze-job-work-input
...
async def check_job_feasibility(job_details: JobDetails):
    ...
    try:
        ...
        if ai_response and "feasible" in ai_response and "reason" in ai_response:
            ...
            raise HTTPException(
                status_code=500,
                detail={"feasible": False, "reason": "Failed to get a valid analysis from AI. Please try again."}
            )
...
async def evaluate_job_fit(request: JobFitRequest):
    ...
    try:
        ...
        if not job_details_from_chain or not job_details_from_chain.get('descriptionIPFSHash'):
            raise HTTPException(status_code=404, detail="Job details not found on the blockchain.")

        # 2. Fetch the job description from IPFS
        job_description_text = ipfs_service.get_file_content_from_ipfs(job_details_from_chain['descriptionIPFSHash'])
        if not job_description_text:
            raise HTTPException(status_code=404, detail="Failed to fetch job description from IPFS.")
        ...
        if ai_response and "match_score" in ai_response and "reason" in ai_response:
            ...
            raise HTTPException(status_code=500, detail="Failed to get a valid analysis from AI.")
...
async def get_freelancer_profile(address: str):
    ...
    MOCK_PROFILES = {
        "0xB9F17DEadBa7257f7ab0EF92df1c7A8799333f4E": FreelancerProfile(
            skills=["Python", "Data Science", "Machine Learning", "FastAPI", "Solidity (Basic)"],
        ...
        "0x71d6208666c17b2AfBfF9A17354292513536ca61": FreelancerProfile( # This is the address from user's logs
            skills=["Web Development", "React", "TypeScript", "Tailwind CSS", "Solidity (Intermediate)"],
    ...
    profile = MOCK_PROFILES.get(address, FreelancerProfile(
...
async def analyze_job_work_input(job_id: int):
    ...
    if USE_MOCK_ANALYZE_INPUT:
        ...
        return {"inputType": "repo_link", "reason": "Mock AI analysis: This job appears to be a code development task."}

    try:
        # 1. Fetch job details from the blockchain
        job_details_from_chain = blockchain_service.get_job_details(job_id)
        if not job_details_from_chain or not job_details_from_chain.get('descriptionIPFSHash'):
            raise HTTPException(status_code=404, detail="Job description not found on the blockchain.")
        ...
        if ai_response and "inputType" in ai_response and "reason" in ai_response:
            ...
            if ai_response["inputType"] not in ["ipfs_hash", "repo_link"]:
                raise HTTPException(status_code=500, detail="AI returned an invalid inputType.")
            return ai_response
        else:
            raise HTTPException(status_code=500, detail="Failed to get a valid analysis from AI.")
...
TRADER_JOE_ROUTER_ADDRESS = "0x18556DA13313f3532c54711497A8FedAC273220E" # Router V2.2 (Liquidity Book)
